`[<-.ibts` <- function(x,i,j,value){
	...
	if(has.i){
		...
		if(!is.name(cl_i)){
			...
			if(deparse(cl_i[[1]]) %in% "-"){
				if(is.character(cl_i[[2]]) || (is.name(cl_i[[2]]) && is.character(eval(cl_i[[2]])))){
					cl_i <- cl_i[[-1]]
					neg <- TRUE
					i <- eval(cl_i)
				} else {
		...
		if(is.character(i)){
			...
			if(any(split_char)){
				i_list[split_char] <- lapply(i[split_char],function(indx,tz,st_index,et_index){
	...
	if(has.i && is.numeric(i) && !all(i[i>0] %in% c(1:nrow(x))))stop("adding rows is not defined for \"ibts\" objects")
